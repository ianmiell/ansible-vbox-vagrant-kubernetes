    ...
    (1..NODES_NUM).each do |j|
        config.vm.define "k8s-n-#{j}" do |node|
            ...
            node.vm.provider :virtualbox do |vb|
                ...
                vb.customize ['createhd', '--filename', "centos65-disk#{j}.vdi", '--size', 10*1024]
                vb.customize ['storageattach', :id, '--storagectl', 'SATA Controller', '--port', 1, '--device', 0, '--type', 'hdd', '--medium', "centos65-disk#{j}.vdi"]
            end
        end
    end
end
